...
def test(model, test_loader, device, latent_dim, n_prior_samples=1000, save_figure=True):
    ...
    with torch.no_grad():
        for x_batch, y_batch in test_loader:
            ...
            z_sample = qz_x.rsample()  # shape: (batch_size, latent_dim)
    ...
    # Concatenate collected samples (z) and labels
    posterior_samples = torch.cat(posterior_samples, dim=0).numpy()
    labels = torch.cat(labels, dim=0).numpy()

    # Randomly select at posterior points to visualize 
    n_show = 700
    ...
    if latent_dim > 2:
        ...
        posterior_2d = pca.fit_transform(posterior_samples)
        prior_2d = pca.transform(z_prior)
    else:
        posterior_2d = posterior_samples
        prior_2d = z_prior

    # Compute kernel density estimation (KDE) over the prior samples ----
    kde = gaussian_kde(prior_2d.T)
    x_min, x_max = posterior_2d[:, 0].min() - 1, posterior_2d[:, 0].max() + 1
    y_min, y_max = posterior_2d[:, 1].min() - 1, posterior_2d[:, 1].max() + 1
    X, Y = np.meshgrid(
    ...
    # Plot the prior (contour) and posterior samples (scatter)
    plt.figure(figsize=(8, 6))

    # Contours for the prior density
    plt.contourf(X, Y, Z, levels=20, cmap="hot_r", alpha=0.7, label="Prior (density)")
    ...
    # Create a colorbar for the posterior labels
    cbar = plt.colorbar(scatter_plot)
    cbar.set_label("Class Label")

    plt.xlabel("z1" if latent_dim <= 2 else "PC1")
    plt.ylabel("z2" if latent_dim <= 2 else "PC2")
    plt.title("MixtureGaussianPrior: Prior + Posterior")
    plt.grid(False)
    plt.legend()

    if save_figure:
        plt.savefig("prior_posterior_MixtureGaussianPrior.png", bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    import sys
        # Arguments
    # sys.argv = [
    #     "vae_bernoulli_MOG.py", "train", 
    #     "--model", "model_Mog.pt", 
    #     "--samples", "model_Mog.png", 
    #     "--device", "cuda", 
    #     "--batch-size", "128", 
    #     "--epochs", "50", 
    #     "--latent-dim", "50",
    ...
    sys.argv = [
        "vae_bernoulli_MOG.py",
        "--mode", "test",
        "--model", "VAE/model_Mog_FNN_20.pt",
        "--samples", "model_MOG.png",
        "--device", "cuda",
        "--batch-size", "128",
        "--epochs", "50",
