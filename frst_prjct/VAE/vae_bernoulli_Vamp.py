...
def test(model, test_loader, device, latent_dim, n_prior_samples=1000, save_figure=True):
    ...
    plt.title("VampPrior: Prior + Posterior")
    plt.grid(False)
    plt.legend()

    if save_figure:
        plt.savefig("prior_posterior_Vamp.png", bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    import sys
        # Arguments
    sys.argv = [
        "vae_bernoulli_Vamp.py", "test", 
        "--model", "VAE\model_Vamp_fnn_20.pt", 
        "--samples", "model_Vamp.png", 
        "--device", "cuda", 
        "--batch-size", "128", 
        "--epochs", "50", 
        "--latent-dim", "20",
    ...
    prior = VampPrior(latent_dim=M, encoder=encoder)
